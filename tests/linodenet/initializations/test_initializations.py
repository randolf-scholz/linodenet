...
class TestCorrectness:
    ...
    @pytest.mark.parametrize(
        "num_samples", [1024], ids=lambda n_samples: f"{n_samples=}"
    )
    ...
    def test_validity_initializations(self, name: str) -> None:
        ...
        matrix = initialization(self.BATCH_SIZE, size, **kwargs)
        assert matrix.shape == (self.BATCH_SIZE, size, size)
        try:
        ...
        assert torch.all(result).item(), (
            f"{name} failed test {matrix_test.__name__}\n{matrix=}."
        )
