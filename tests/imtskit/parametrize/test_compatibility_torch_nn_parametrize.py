r"""Check that the imtskit.parametrizations module is compatible with torch.nn.utils.parametrizations."""
...
def test_signatures_compatibility_torch(name: str) -> None:
    impl = getattr(imtskit.nn.parametrize, name, None)
    ...
    if impl is None:
        pytest.xfail(f"imtskit.parametrizations.{name} not implemented")
