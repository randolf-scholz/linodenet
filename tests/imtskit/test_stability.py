r"""Test whether `LinODEnet` is forward stable."""

import logging

import pytest
import torch

from imtskit.forecasting import LinODEnet
from imtskit.mappings import embeddings
from imtskit.nn import ResNet
from imtskit.state_propagation import LinearFlow
from imtskit.state_update import CellSequence


@pytest.mark.skip(reason="Not implemented yet.")
def test_model_stability() -> None:
    r"""TODO: Implement this test."""
    logger = logging.getLogger(f"{__name__}/{LinODEnet.__name__}")
    logger.info("Testing stability.")

    N, D, L = 1000, 5, 32
    MODEL_CONFIG = {
        "__name__": "LinODEnet",
        "input_size": D,
        "hidden_size": L,
        "embedding_type": "concat",
        "Filter": CellSequence,
        "System": {
            "__module__": LinearFlow.__module__,
            "__name__": LinearFlow.__qualname__,
            "kernel_initialization": "skew-symmetric",
        },
        "Encoder": ResNet,
        "Decoder": ResNet,
        "Embedding": embeddings.ConcatEmbedding,
    }
    print(MODEL_CONFIG)
    T = torch.randn(N)
    X = torch.randn(N, D)
    model = LinODEnet(D, L)
    model(T, X)
