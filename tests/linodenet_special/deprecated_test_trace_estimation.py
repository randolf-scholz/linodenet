...
ESTIMATORS = {
    ...
    "xtrace-correction": xtrace_estimator_matlab,
    ...
    def compute_estimate(self, method: str, A: Tensor, x: Tensor) -> Tensor:
        match method:
            ...
            case "xtrace-correction":
                assert ESTIMATORS[method] is xtrace_estimator_matlab
                return xtrace_estimator_matlab(
                    lambda v: torch.einsum("...nd, ...md -> ...nm", v, A), x
                )
            case "btrace":
...
def test_xtrace_estimator_corrected_single_and_batched(device: str) -> None:
    ...
    batched_estimate = xtrace_estimator_matlab(
...
class TestLogAbsDetEstimator(TestSuite):
    ...
    def test_logabsdet_estimator_dispatch(
        ...
        estimator = LogabsdetSeriesEstimator(method, num_samples, num_series_terms)
    ...
    def test_exact_estimator_matches_closed_form(self, device: str) -> None:
        ...
        estimator = LogabsdetSeriesEstimator("exact", None, None).to(device=device)
