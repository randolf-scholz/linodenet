...
class TriangularTransform(nn.Module, Transform):
    ...
    def config(self) -> dict[str, int | tuple[int, ...]]:
        return {
            ...
            "permutation": tuple(int(x) for x in self.permutation.tolist()),
        }
    ...
    def __init__(
        ...
        inv_perm[perm] = torch.arange(input_size, device=perm.device, dtype=perm.dtype)
        self.register_buffer("permutation", perm, persistent=True)
        self.register_buffer("inverse_permutation", inv_perm, persistent=True)
