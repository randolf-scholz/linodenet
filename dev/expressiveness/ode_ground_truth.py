...
if __name__=="__main__":
    ...
    "Encoder": ResNet.HP | {"num_blocks":3}
    }
   ...
   # }
    ...
    for fold,(train_index, test_index) in enumerate(rs.split(data_t)):
        writer = SummaryWriter(f"./.logs/r6_{LATENT}_{HIDDEN}/")
        #writer.add_hparams(HP,{})
        last_test_loss = 1e20
        counts = 0
        model = LinODEnet(DIM,LATENT,HIDDEN, **HP).to(DEVICE)
        t_train = torch.Tensor(data_t[train_index]).type(DTYPE)
        x_train = torch.Tensor(data_x[train_index]).type(DTYPE)
        x_train_past = torch.ones_like(x_train).copy_(x_train)
        past = np.random.randint(PAST-50, PAST+50)
        x_train_past[:,past:,:] = torch.nan
        t_test = torch.Tensor(data_t[test_index]).type(DTYPE)
        x_test = torch.Tensor(data_x[test_index]).type(DTYPE)
        x_test_past = torch.ones_like(x_test).copy_(x_test)
